...
범위: 랜딩·튜토리얼·교수 연결·대화 준비·AI 프로젝트·맞춤 교수 추천·면담 후 행동·성장 기록
...
`홈 → 교수 매칭 → 교수 만남 준비 → AI 프로젝트 설계 → 맞춤 교수 추천 → 나의 성장과정`

- 첫 여정은 학생의 고민을 기준으로 교수를 매칭하고 실제 만남을 준비한다.
- 두 번째 여정은 AI와 프로젝트를 설계한 뒤 선택한 프로젝트의 주제·방법·응용 맥락에 맞는 교수를 추천한다.
- 맞춤 교수 추천 탭은 프로젝트가 없으면 설계 시작, 후보만 있으면 주제 선택, 선택까지 끝났으면 추천 근거로 이어져 빈 화면을 만들지 않는다.
...
| 아이디어 후보 비교 | `/result` | 조건·공동설계 답변 | 근거와 확인 항목이 있는 후보 2개 | 프로젝트 멘토 연결 |
| 프로젝트 멘토 연결 | `/result#professor-connection` | 학생이 고른 아이디어·공식 교수 후보 | AI 재정렬한 주제·방법·확장 관점 교수 3명 | 교수 상세·첫 질문 |
| 맞춤 교수 추천 허브 | `/project-professors` | 프로젝트·선택 주제·프로젝트 교수 결과 | 상태별 다음 행동·역할별 교수 요약 | 프로젝트 설계·후보 선택·추천 근거 |
