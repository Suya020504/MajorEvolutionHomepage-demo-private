...
| 현재 Q01 MVP | `components/paper-reader/paper-reader-shell.tsx` | 텍스트 입력·구조화 분석·카드 편집·복사·퀘스트 저장 |
| 즐겨찾기 논문 선택 | `components/paper-reader/favorite-professor-paper-picker.tsx` | 즐겨찾는 교수·공식 논문 선택, 제목 검색, 연도 필터 |
| 공식 논문 API | `app/api/professors/favorites/route.ts` | 저장된 교수 ID를 공식 데이터로 다시 조회하고 최근 논문 최대 6건 제공 |
| 논문 선택 규칙 | `lib/professor-paper-selection.ts` | 최신순 정렬, 검색·연도 필터, 교수·논문 ID가 결합된 선택 메타데이터 |
| 즐겨찾기·선택 상태 | `store/research-store.ts` | 교수 ID 목록과 현재 선택 논문을 로컬 저장, v2→v3 기존 연구 결과 보존 |
| 3분 카드 묶음 | `store/quest-store.ts` | 논문별 5카드 단일 저장·재저장 시 갱신, v1 카드 보존 |
| 공통 계약 | `lib/paper-reader-contract.ts` | 입력·저장 결과 타입의 단일 기준 |
| 기존 API | `app/api/ai/paper/route.ts` | 텍스트 구조화 분석, 호환 유지 |
| 구형 주소 | `app/paper/page.tsx` | `/paper/reader?mode=bite`로 이동 |

현재 Q01은 사용자가 붙여 넣은 80~12,000자의 텍스트를 문제·방법·결과·한계·질문 카드로 정리한다. 사용자는 카드를 편집하거나 복사하고, 교수님 퀘스트 기록에 저장할 수 있다. 같은 분석 결과를 다시 저장하면 기존 카드를 수정해 중복을 막는다.

현재 결과는 `사용자가 붙여 넣은 텍스트 범위 · 페이지 정보 없음`으로 표시한다. PDF를 읽었거나 페이지 근거가 있는 것처럼 표현하지 않는다. 담당 개발자는 이 화면의 입력·결과 경계를 유지하면서 PDF 원문, 페이지 단위 근거, 번역, 자유 질의응답, 그림 해설 모듈을 단계적으로 교체 연결한다.
...
→ 교수님 퀘스트 > 논문 한입
→ 즐겨찾는 교수 선택
→ 공식 프로필 수집 최근 논문 최대 6건에서 선택
→ 공식 제목·발행일·서지 출처 자동 입력
→ 학생이 같은 논문의 초록/본문을 직접 붙여넣고 일치 여부 확인
→ 3분 카드 생성
→ 같은 교수·같은 논문은 기존 5카드 묶음 갱신
```

- 논문 선택만으로 요약을 만들지 않는다. 현재 공식 데이터에는 모든 초록·원문이 없기 때문이다.
- 새로고침 후 저장된 제목과 URL을 바로 신뢰하지 않는다. `professorId + paperId`를 공식 API로 다시 확인한 뒤에만 `공식 프로필 서지정보`로 표시한다.
- 네트워크 재확인 실패 시 저장된 선택을 지우지 않고 재시도·다른 논문·직접 입력을 제공한다.
- 공식 목록에서 사라진 논문은 선택을 해제하고 다시 고르도록 안내한다.
- 즐겨찾기는 현재 브라우저에만 저장된다. 로그인·기기 간 동기화는 후속 범위다.
